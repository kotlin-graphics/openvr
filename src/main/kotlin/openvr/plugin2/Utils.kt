...
val HmdMatrix34.rotation: Quat
    get() = when {
        (m(2) != 0f || m(6) != 0f || m(10) != 0f) && (m(1) != 0f || m(5) != 0f || m(9) != 0f) ->
            glm.quatLookAt(Vec3(-m(2), -m(6), m(10)).normalizeAssign(), Vec3(m(1), m(5), -m(9)))
        else -> Quat()
    }

val HmdMatrix34.position: Vec3
    get() = Vec3(m(3), m(7), -m(11))
...
//    constructor(Transform fromTransform)
//    {
...
//    }
    ...
    constructor(pose: HmdMatrix34) {
        ...
        val matrix = Mat4()
        // indices are inverted because Unity uses [row, column]
        matrix[0, 0] = pose.m(0)
        matrix[1, 0] = pose.m(1)
        matrix[2, 0] = -pose.m(2)
        matrix[3, 0] = pose.m(3)
        ...
        matrix[0, 2] = -pose.m(8)
        matrix[1, 2] = -pose.m(9)
        matrix[2, 2] = pose.m(10)
        matrix[3, 2] = -pose.m(11)
    ...
    constructor(pose: HmdMatrix44) {
        ...
        matrix[0, 3] = pose.m(12)
        matrix[1, 3] = pose.m(13)
        matrix[2, 3] = -pose.m(14)
        matrix[3, 3] = pose.m(15)

        matrix[3].to(pos)
        matrix to rot
    }

//    val asHmdMatrix44: HmdMatrix44
//        get() {
//            val matrix = Mat4().translate(pos) * rot.toMat4()
//            return HmdMatrix44.callocStack().apply {
//                m(0, matrix[0, 0])
//                m(1, matrix[0, 1])
//                m(2, -matrix[0, 2])
//                m(3, matrix[0, 3])
