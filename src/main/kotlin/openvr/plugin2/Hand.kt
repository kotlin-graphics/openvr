...
infix fun Int.or(f: Hand.AttachmentFlag) = or(f.i)
 ...
 *  ------------------------------------------------------------------------- */
class Hand(
        val handType: SteamVR_Input_Sources
) {
    ...
    enum class AttachmentFlag {
        ...
        SnapOnAttach,
        /** Other objects attached to this hand will be detached. */
        DetachOthers,
        /** This object will be detached from the other hand. */
        DetachFromOtherHand,
        /** The object will be parented to the hand. */
        ParentToHand,
        /** The object will attempt to move to match the position and rotation of the hand. */
        VelocityMovement,
        /** The object will not respond to external physics. */
        TurnOnKinematic,
        /** The object will not respond to external physics. */
        TurnOffGravity,
        /** The object is able to switch from a pinch grab to a grip grab.
         *  Decreases likelyhood of a good throw but also decreases likelyhood of accidental drop */
        AllowSidegrade;

        val i = 1 shl ordinal

        infix fun or(f: AttachmentFlag) = i or f.i
    }

    val defaultAttachmentFlags: AttachmentFlags = AttachmentFlag.ParentToHand or AttachmentFlag.DetachOthers or
            AttachmentFlag.DetachFromOtherHand or AttachmentFlag.TurnOnKinematic or AttachmentFlag.SnapOnAttach

    lateinit var otherHand: Hand

    lateinit var trackedObject: SteamVR_Behaviour_Pose

    val grabPinchAction = SteamVR_Input.getAction<SteamVR_Action_Boolean>(ActionType.Boolean, "GrabPinch")

    val grabGripAction = SteamVR_Input.getAction<SteamVR_Action_Boolean>(ActionType.Boolean, "GrabGrip")

    val hapticAction = SteamVR_Input.getAction<SteamVR_Action_Vibration>(ActionType.Vibration, "Haptic")

    val uiInteractAction = SteamVR_Input.getAction<SteamVR_Action_Boolean>(ActionType.Boolean, "InteractUI")

    var useHoverSphere = true
    var hoverSphereTransform: Transform? = null
    var hoverSphereRadius = 0.05f
//    public LayerMask hoverLayerMask = -1
    var hoverUpdateInterval = 0.1f

    var useControllerHoverComponent = true
    val controllerHoverComponent = "tip"
    var controllerHoverRadius = 0.075f

    var useFingerJointHover = true
    val fingerJointHover = JointIndex.indexTip
    var fingerJointHoverRadius = 0.025f
...
//    {
//        if (noSteamVRFallbackCamera) {
//            Ray ray = noSteamVRFallbackCamera . ScreenPointToRay (SteamVR_Input.mousePosition)
//
//            if (attachedObjects.Count > 0) {
...
//            if (noSteamVRFallbackCamera) {
//                if (SteamVR_Input.GetMouseButtonDown(0))
//                    return explicitType
//            }
//
...
//        if (explicitType != GrabTypes.None) {
//            if (noSteamVRFallbackCamera) {
//                if (SteamVR_Input.GetMouseButtonUp(0))
//                    return explicitType
//            }
//
...
//    {
//        if (noSteamVRFallbackCamera) {
//            if (SteamVR_Input.GetMouseButton(0))
//                return true
//        }
//
//        switch(type)
