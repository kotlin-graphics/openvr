...
open class Transform : Component() {
    ...
    fun getLocalEulerAngles(order: RotationOrder): Vec3 = TODO()

//    /** Set local euler angles with rotation order specified */
//    fun setLocalEulerAngles(euler: Vec3, order: RotationOrder ) = TODO()
//
//    /** Set local euler hint */
//    fun setLocalEulerHint(euler: Vec3) = TODO()
//
//    /** The rotation as Euler angles in degrees. */
//    val eulerAngles: Vec3
//        get() = rotation.eulerAngles
//        set(value) { rotation = Quaternion.Euler(value); } }
//
    ...
    var parent: Transform
        ...
        set(value) {
            if (this is RectTransform)
                println("Parent of RectTransform is being set with parent property. Consider using the SetParent method instead, with the worldPositionStays argument set to false. This will retain local orientation and scale rather than world orientation and scale, which can prevent common UI scaling issues.")
            parentInternal = value
        }

    var parentInternal: Transform
        get() = TODO()
        set(value) = setParent(value)

    fun setParent(parent: Transform, worldPositionStays: Boolean = true) {

    }

    // Matrix that transforms a point from world space into local space (RO).
    var worldToLocalMatrix = Mat4()
    // Matrix that transforms a point from local space into world space (RO).
    var localToWorldMatrix = Mat4()
...
//    public Vector3 InverseTransformVector(float x, float y, float z) { return InverseTransformVector(new Vector3(x, y, z)); }
    ...
    fun transformPoint(x: Float, y: Float, z: Float): Vec3 = transformPoint(Vec3(x, y, z))
