 ...
 */
val IVRSystem.recommendedRenderTargetSize: Vec2i
    get() {
        val pWidth = appBuffer.int
        val pHeight = appBuffer.int
        VRSystem.nVRSystem_GetRecommendedRenderTargetSize(pWidth, pHeight)
        return Vec2i(memGetInt(pWidth), memGetInt(pHeight))
    }
 ...
 */
fun IVRSystem.getProjectionMatrix(eye: EVREye, nearZ: Float, farZ: Float, res: Mat4 = Mat4()): Mat4 {
    val hmdMat44 = vr.HmdMatrix44()
    VRSystem.nVRSystem_GetProjectionMatrix(eye.i, nearZ, farZ, hmdMat44.adr)
    val m = hmdMat44.m
    return res(
            m[0], m[4], m[8], m[12],
            m[1], m[5], m[9], m[13],
 ...
 */
fun IVRSystem.getProjectionRaw(eye: EVREye, left: FloatBuffer, right: FloatBuffer, top: FloatBuffer, bottom: FloatBuffer) {
    VRSystem.nVRSystem_GetProjectionRaw(eye.i, left.adr, right.adr, top.adr, bottom.adr);
}
 ...
 */
fun IVRSystem.computeDistortion(eye: EVREye, u: Float, v: Float, distortionCoordinates: DistortionCoordinates): Boolean {
    return VRSystem.nVRSystem_ComputeDistortion(eye.i, u, v, distortionCoordinates.adr)
}
 ...
 */
fun IVRSystem.getEyeToHeadTransform(eye: EVREye, res: Mat4 = Mat4()): Mat4 {
    val hmdMat34 = HmdMatrix34.create(appBuffer.ptr.advance(HmdMatrix34.SIZEOF))
    VRSystem.nVRSystem_GetEyeToHeadTransform(eye.i, hmdMat34.adr)
    val m = hmdMat34.m
    return res(
            m[0], m[4], m[8], 0f,
            m[1], m[5], m[9], 0f,
 ...
 */
fun IVRSystem.getTimeSinceLastVsync(secondsSinceLastVsync: FloatBuffer, frameCounter: LongBuffer): Boolean {
    return VRSystem.nVRSystem_GetTimeSinceLastVsync(secondsSinceLastVsync.adr, frameCounter.adr)
}
 ...
 */
fun IVRSystem.getD3D9AdapterIndex(): Int = VRSystem.VRSystem_GetD3D9AdapterIndex()
 ...
 */
infix fun IVRSystem.getDXGIOutputInfo(adapterIndex: IntBuffer) = VRSystem.nVRSystem_GetDXGIOutputInfo(adapterIndex.adr)
 ...
 */
fun IVRSystem.getOutputDevice(device: LongBuffer, textureType: TextureType, instance: VkInstance? = null) {
    VRSystem.nVRSystem_GetOutputDevice(device.adr, textureType.i, instance?.adr ?: NULL)
}
...
val IVRSystem.isDisplayOnDesktop: Boolean
    get() = VRSystem.VRSystem_IsDisplayOnDesktop()
 ...
 */
infix fun IVRSystem.setDisplayVisibility(isVisibleOnDesktop: Boolean): Boolean {
    return VRSystem.VRSystem_SetDisplayVisibility(isVisibleOnDesktop)
}
 ...
 */
fun IVRSystem.resetSeatedZeroPose() = VRSystem.VRSystem_ResetSeatedZeroPose()
 ...
 */
val IVRSystem.seatedZeroPoseToStandingAbsoluteTrackingPose: Mat4
    get() {
        val hmdMat34 = vr.HmdMatrix34()
        VRSystem.nVRSystem_GetSeatedZeroPoseToStandingAbsoluteTrackingPose(hmdMat34.adr)
        val m = hmdMat34.m
        return Mat4(
                m[0], m[4], m[8], 0f,
                m[1], m[5], m[9], 0f,
 ...
 */
val IVRSystem.rawZeroPoseToStandingAbsoluteTrackingPose: Mat4
    get() {
        val hmdMat34 = vr.HmdMatrix34()
        VRSystem.nVRSystem_GetRawZeroPoseToStandingAbsoluteTrackingPose(hmdMat34.adr)
        val m = hmdMat34.m
        return Mat4(
                m[0], m[4], m[8], 0f,
                m[1], m[5], m[9], 0f,
 ...
 * @param deviceId
 */
infix fun IVRSystem.getTrackedDeviceActivityLevel(deviceId: TrackedDeviceIndex): EDeviceActivityLevel {
    return EDeviceActivityLevel of VRSystem.VRSystem_GetTrackedDeviceActivityLevel(deviceId)
}
 ...
 * @param transform
 */
fun IVRSystem.applyTransform(outputPose: TrackedDevicePose, trackedDevicePose: TrackedDevicePose, transform: Mat4) {
    val hmdMat34 = vr.HmdMatrix34()
    hmdMat34.m.put(0, transform[0, 0]).put(1, transform[0, 1]).put(2, transform[0, 2]).put(3, transform[0, 3])
            .put(4, transform[1, 0]).put(5, transform[1, 1]).put(6, transform[1, 2]).put(7, transform[1, 3])
            .put(8, transform[2, 0]).put(9, transform[2, 1]).put(10, transform[2, 2]).put(11, transform[2, 3])
    VRSystem.nVRSystem_ApplyTransform(outputPose.adr, trackedDevicePose.adr, hmdMat34.adr)
}
 ...
 */
infix fun IVRSystem.getTrackedDeviceIndexForControllerRole(deviceType: TrackedControllerRole): TrackedDeviceIndex {
    return VRSystem.VRSystem_GetTrackedDeviceIndexForControllerRole(deviceType.i)
}
 ...
 * @param deviceIndex
 */
infix fun IVRSystem.getControllerRoleForTrackedDeviceIndex(deviceIndex: TrackedDeviceIndex): TrackedControllerRole {
    return TrackedControllerRole of VRSystem.VRSystem_GetControllerRoleForTrackedDeviceIndex(deviceIndex)
}
 ...
 */
infix fun IVRSystem.getTrackedDeviceClass(deviceIndex: TrackedDeviceIndex): TrackedDeviceClass {
    return TrackedDeviceClass of VRSystem.VRSystem_GetTrackedDeviceClass(deviceIndex)
}
 ...
 */
infix fun IVRSystem.isTrackedDeviceConnected(deviceIndex: TrackedDeviceIndex): Boolean {
    return VRSystem.VRSystem_IsTrackedDeviceConnected(deviceIndex)
}

inline fun <reified T> IVRSystem.getTrackedDeviceProperty(deviceIndex: TrackedDeviceIndex, prop: TrackedDeviceProperty, error: IntBuffer? = null): T {
    return when (T::class.java) {
        Boolean::class.java -> getBoolTrackedDeviceProperty(deviceIndex, prop, error) as T
        Float::class.java -> getFloatTrackedDeviceProperty(deviceIndex, prop, error) as T
        Int::class.java -> getIntTrackedDeviceProperty(deviceIndex, prop, error) as T
        Long::class.java -> getLongTrackedDeviceProperty(deviceIndex, prop, error) as T
        Mat4::class.java -> getMat4TrackedDeviceProperty(deviceIndex, prop, error) as T
    // TODO getArray
        else -> throw Error()
    }
}
 ...
 * @param error        the error returned when attempting to fetch this property. This can be {@code NULL} if the caller doesn't care about the source of a property error.
 */
fun IVRSystem.getBoolTrackedDeviceProperty(deviceIndex: TrackedDeviceIndex, prop: TrackedDeviceProperty, error: IntBuffer? = null): Boolean {
    return VRSystem.nVRSystem_GetBoolTrackedDeviceProperty(deviceIndex, prop.i, error?.adr ?: NULL)
}
 ...
 * @param error        the error returned when attempting to fetch this property. This can be {@code NULL} if the caller doesn't care about the source of a property error.
 */
fun IVRSystem.getFloatTrackedDeviceProperty(deviceIndex: TrackedDeviceIndex, prop: TrackedDeviceProperty, error: IntBuffer? = null): Float {
    return VRSystem.nVRSystem_GetFloatTrackedDeviceProperty(deviceIndex, prop.i, error?.adr ?: NULL)
}
 ...
 * @param error        the error returned when attempting to fetch this property. This can be {@code NULL} if the caller doesn't care about the source of a property error.
 */
fun IVRSystem.getIntTrackedDeviceProperty(deviceIndex: TrackedDeviceIndex, prop: TrackedDeviceProperty, error: IntBuffer? = null): Int {
    return VRSystem.nVRSystem_GetInt32TrackedDeviceProperty(deviceIndex, prop.i, error?.adr ?: NULL)
}
 ...
 * @param error        the error returned when attempting to fetch this property. This can be {@code NULL} if the caller doesn't care about the source of a property error.
 */
fun IVRSystem.getLongTrackedDeviceProperty(deviceIndex: TrackedDeviceIndex, prop: TrackedDeviceProperty, error: IntBuffer? = null): Long {
    return VRSystem.nVRSystem_GetUint64TrackedDeviceProperty(deviceIndex, prop.i, error?.adr ?: NULL)
}
 ...
 * @param error        the error returned when attempting to fetch this property. This can be {@code NULL} if the caller doesn't care about the source of a property error.
 */
fun IVRSystem.getMat4TrackedDeviceProperty(deviceIndex: TrackedDeviceIndex, prop: TrackedDeviceProperty, error: IntBuffer? = null): Mat4 {
    val hmdMat34 = vr.HmdMatrix34()
    VRSystem.nVRSystem_GetMatrix34TrackedDeviceProperty(deviceIndex, prop.i, error?.adr ?: NULL, hmdMat34.adr)
    val m = hmdMat34.m
    return Mat4(
            m[0], m[4], m[8], 0f,
            m[1], m[5], m[9], 0f,
 ...
 * @param error        the error returned when attempting to fetch this property. This can be {@code NULL} if the caller doesn't care about the source of a property error.
 */
fun IVRSystem.getArrayTrackedDeviceProperty(deviceIndex: TrackedDeviceIndex, prop: TrackedDeviceProperty, propType: PropertyTypeTag, buffer: ByteBuffer?, error: IntBuffer? = null): Int {
    return VRSystem.nVRSystem_GetArrayTrackedDeviceProperty(deviceIndex, prop.i, propType, buffer?.adr
            ?: NULL, buffer?.remaining ?: 0, error?.adr ?: NULL)
}
 ...
 */
fun IVRSystem.getStringTrackedDeviceProperty(deviceIndex: TrackedDeviceIndex, prop: TrackedDeviceProperty, error: IntBuffer? = null): String {

    val pErr = error ?: appBuffer.intBuffer
    val initialSize = 32
    val bytes = appBuffer.buffer(initialSize)
    val propLen = VRSystem.nVRSystem_GetStringTrackedDeviceProperty(deviceIndex, prop.i, bytes.adr, initialSize, pErr.adr)

    return when (pErr[0]) {
        TrackedPropertyError.Success.i -> String(bytes.toByteArray()).replace("$NUL", "")
        TrackedPropertyError.BufferTooSmall.i -> {
            val newBytes = appBuffer.buffer(propLen)
            VRSystem.nVRSystem_GetStringTrackedDeviceProperty(deviceIndex, prop.i, newBytes.adr, initialSize, pErr.adr)
            String(newBytes.toByteArray()).replace("$NUL", "")
        }
        else -> throw Error(TrackedPropertyError.of(pErr[0]).toString())
    }
}
 ...
 * @param size the size in bytes of the {@link VREvent} struct
 */
fun IVRSystem.pollNextEvent(event: VREvent, size: Int = VREvent.SIZEOF): Boolean {
    return VRSystem.nVRSystem_PollNextEvent(event.adr, size)
}
 ...
 */
fun IVRSystem.pollNextEventWithPose(origin: TrackingUniverseOrigin, event: VREvent, trackedDevicePose: TrackedDevicePose): Boolean {
    return VRSystem.nVRSystem_PollNextEventWithPose(origin.i, event.adr, VREvent.SIZEOF, trackedDevicePose.adr)
}
 ...
 */
fun IVRSystem.getHiddenAreaMesh(eye: EVREye, type: EHiddenAreaMeshType = EHiddenAreaMeshType.Standard): HiddenAreaMesh {
    return vr.HiddenAreaMesh().apply {
        VRSystem.nVRSystem_GetHiddenAreaMesh(eye.i, type.i, adr)
    }
}
 ...
 */
fun IVRSystem.getControllerState(controllerDeviceIndex: TrackedDeviceIndex, controllerState: VRControllerState): Boolean {
    return VRSystem.nVRSystem_GetControllerState(controllerDeviceIndex, controllerState.adr, VRControllerState.SIZEOF)
}
 ...
 */
fun IVRSystem.getControllerStateWithPose(origin: TrackingUniverseOrigin, controllerDeviceIndex: TrackedDeviceIndex, controllerState: VRControllerState, trackedDevicePose: TrackedDevicePose): Boolean {
    return VRSystem.nVRSystem_GetControllerStateWithPose(origin.i, controllerDeviceIndex, controllerState.adr, VRControllerState.SIZEOF, trackedDevicePose.adr)
}
 ...
 */
fun IVRSystem.triggerHapticPulse(controllerDeviceIndex: TrackedDeviceIndex, axisId: Int, durationMicroSec: Short) {
    VRSystem.VRSystem_TriggerHapticPulse(controllerDeviceIndex, axisId, durationMicroSec)
}
 ...
 */
val IVRSystem.isInputAvailable: Boolean
    get() = VRSystem.VRSystem_IsInputAvailable()
 ...
 * hands in this case.
 */
val IVRSystem.isSteamVRDrawingControllers: Boolean
    get() = VRSystem.VRSystem_IsSteamVRDrawingControllers()
 ...
 */
val IVRSystem.shouldApplicationPause: Boolean
    get() = VRSystem.VRSystem_ShouldApplicationPause()
 ...
 */
val IVRSystem.shouldApplicationReduceRenderingWork: Boolean
    get() = VRSystem.VRSystem_ShouldApplicationReduceRenderingWork()
 ...
 */
fun IVRSystem.driverDebugRequest(deviceIndex: TrackedDeviceIndex, request: String, responseBufferSize: Int = vr.maxDriverDebugResponseSize): String {
    val requestEncoded = appBuffer.bufferOfAscii(request)
    val responseBuffer = appBuffer.buffer(responseBufferSize)
    val size = VRSystem.nVRSystem_DriverDebugRequest(deviceIndex, requestEncoded.adr, responseBuffer.adr, responseBufferSize)
 ...
 * @param deviceIndex
 */
infix fun IVRSystem.performFirmwareUpdate(deviceIndex: TrackedDeviceIndex): EVRFirmwareError {
    return EVRFirmwareError of VRSystem.VRSystem_PerformFirmwareUpdate(deviceIndex)
}
 ...
 */
fun IVRSystem.acknowledgeQuit_Exiting() = VRSystem.VRSystem_AcknowledgeQuit_Exiting()
 ...
 */
fun IVRSystem.acknowledgeQuit_UserPrompt() = VRSystem.VRSystem_AcknowledgeQuit_UserPrompt()