...
class Application {
    ...
    init {
        if (debugOpenGL) {
            glDebugMessageCallback { source, type, severity, id, message ->
                val string = "$id: $type of $severity severity, raised from $source: $message"
                if (severity == GlDebugSeverity.HIGH)
                    throw Error(string)
                else
                    println(string)
            }
    ...
    fun handleInput() {
        ...
        for (hand in Hand.values()) {
            ...
            val err = vrInput.getPoseA ctionData(rHand[hand].actionPose, TrackingUniverseOrigin.Standing, 0f, poseData, vr.invalidInputValueHandle)
